...
// register set 1 includes TDI, TCK
// register set 3 includes TMS, TDO
#define GPLR1 0x0004/4 // GPIO pin level register
#define GPLR3 0x0100/4
#define GPDR1 0x0010/4 // GPIO pin direction register
#define GPDR3 0x010c/4
#define GPSR1 0x001c/4 // GPIO pin set register
#define GPSR3 0x0118/4
#define GPCR1 0x0028/4 // GPIO pin clear register
#define GPCR3 0x0124/4

static unsigned int gpiostate;
extern unsigned int *mvgpioregs, *mvmfpregs;
...
#else
// Hack: writePort_fast will only use TDI, not TMS.
// register set 1 includes TDI, TCK
// register set 3 includes TMS, TDO
...
do {							\
	...
	if ((a_ucValue)) v |= (a_ucPins);		\
        ...
        offset = (a_ucValue) ? GPSR1 : GPCR1;           \
							\
	if (v != ov) mvgpioregs[offset] = (a_ucPins);	\
	(pins) = v;					\
} while(0)

#define sclock_fast(pins) 				\
do {							\
        mvgpioregs[GPSR1] = g_ucPinTCK;                 \
        mvgpioregs[GPCR1] = g_ucPinTCK;                 \
} while(0)

static signed char ispVMSend( unsigned short a_usiDataSize )
{
	unsigned int iIndex;
	//unsigned int iInDataIndex = 0;
	unsigned char cCurByte = 0, *ptr;
...
static const unsigned int g_ucPinTDI     = 1<<15;   /* Bit address of TDI */
static const unsigned int g_ucPinTCK     = 1<<21;   /* Bit address of TCK */
static const unsigned int g_ucPinTMS     = 1<<13;   /* Bit address of TMS */
...
static const unsigned int g_ucPinTDO     = 1<<23;   /* Bit address of TDO*/

// register set 1 includes TDI, TCK
// register set 3 includes TMS, TDO

static void hardware_init(void) {
        // 1 reg for each pin, 0x4000 is std setting for gpio with pullup
        mvmfpregs[0x28/4] = 0x4000;
        mvmfpregs[0x40/4] = 0x4000;
        mvmfpregs[0x1b4/4] = 0x4000;
        mvmfpregs[0x1dc/4] = 0x4000;

        mvgpioregs[GPDR1] |= g_ucPinTDI | g_ucPinTCK;
        mvgpioregs[GPDR3] |= g_ucPinTMS;
        mvgpioregs[GPDR3] &= ~g_ucPinTDO;
}
static void hardware_restore(void) {
        mvgpioregs[GPDR1] &= ~(g_ucPinTDI | g_ucPinTCK);
        mvgpioregs[GPDR3] &= ~g_ucPinTMS;
...
static inline void writePort(int pins, int val) {
	...
	if (val) gpiostate |= pins;
        ...
        x = pins & (g_ucPinTDI | g_ucPinTCK);
        if (x) {
                if (val) mvgpioregs[GPSR1] = x;
                else mvgpioregs[GPCR1] = x;
        }
        x = pins & (g_ucPinTMS);
        if (x) {
                if (val) mvgpioregs[GPSR3] = pins;
                else mvgpioregs[GPCR3] = pins;
        }
}

static inline int readPort() {
	return (mvgpioregs[GPLR3] & g_ucPinTDO) ? 1 : 0;
} 
