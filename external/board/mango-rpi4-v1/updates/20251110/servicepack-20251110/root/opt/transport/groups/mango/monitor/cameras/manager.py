...
#
#   2025-11-10  Todd Valentic
#               Add parameter for camera boot time
#               Extend boot time default from 3s to 10s when mapping
#               Map cameras if mapping file exists but is empty
#
##########################################################################
...
class MonitorManager(ProcessClient, DirectoryMixin):
    ...
    def __init__(self,argv):
        ...
        self.camera_boottime = self.getDeltaTime('camera.boottime', 10)
    ...
    def map_cameras(self):
        ...
        for camera_name in self.monitors:
            ...
            self.wait(self.camera_boottime.total_seconds())
            try:
    ...
    def get_camera_mapping(self):
        ...
        # Use the current mapping if the file exists and there are no errors
        ...
        # Use the mapping if it isn't empty

        if self.camera_map:
            return True

        # Otherwise, we need to map the cameras 
